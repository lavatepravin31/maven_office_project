...
public class Phonenumber_user_enter_less_than_actual_pom {
WebDriver driver;
	...
	public Phonenumber_user_enter_less_than_actual_pom(WebDriver driver) {
		this.driver=driver;
	}
	...
	public void enterDATA() throws InterruptedException {
		...
		phonenumber.sendKeys("1000");
		Reporter.log("Enter less than actaul phone number ",true);
		Thread.sleep(4000);
	...
	public void phonenumber_field_error_message() {
	       if (phonenumber_field_errorMessage.isDisplayed()) {
	    	   ...
	    	   System.out.println("Error message is  :: "+mesg);
				Reporter.log("Error messsage is   "+mesg,true);	

				System.out.println("Email field Error message is showing when user enter less than actual phone number " );
				Reporter.log("passed- Email field Error message is showing when user enter less than actual phone number  ",true);	
			}
			else {
				System.out.println("Email field  Error message is not showing ");
				Reporter.log("failed-Email field Error message is showing when user enter less than actual phone number  ",true);	
