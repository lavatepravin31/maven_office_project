...
public class city_dropdown_blank_error_message {
...
WebDriver driver;
	...
	@FindBy(xpath = "//mat-error[text()='Select City']")
	WebElement citydropdown_errorMessage;
	
	
	public city_dropdown_blank_error_message(WebDriver driver) {
		this.driver=driver;
	}
...
//		s.selectByVisibleText("India");
		...
		driver.findElement(By.xpath("//select[@formcontrolname='city']")).click();
		

		Reporter.log("country drop down is not selected ");
		Reporter.log("state drop down is not selected ");
		Reporter.log("city drop down is not selected ");
...
//		Thread.sleep(2000);
	...
	public void citydropdown_error_message() {
       if (citydropdown_errorMessage.isDisplayed()) {
    	   String mesg = citydropdown_errorMessage.getText();
    	   System.out.println("Error message is  :: "+mesg);
			
			System.out.println("city Error message is showing when its blank " );
				
		}
		else {
			System.out.println("city Error message is not showing ");
		}
       

       String expected_result="Select City";
       String mesg = citydropdown_errorMessage.getText();
       ...
       Reporter.log("Pass- Verify that  without selecting city ,  its should  show error message.",true);		
