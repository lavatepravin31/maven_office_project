...
public class Email_Field_userEnter_interger_spe_symbol {
WebDriver driver;
	...
	WebElement email_field_errorMessage;
	
	
	
	public Email_Field_userEnter_interger_spe_symbol(WebDriver driver) {
		this.driver=driver;
	}
	...
	public void enterDATA() throws InterruptedException {
		...
		Emailid.sendKeys("@123#$%^%$");
		//Emailid.sendKeys("pravinl@nu10.co");
		Reporter.log("Enter Intger value in Email address field",true);
	...
	public void email_field_error_message() {
		boolean status = continuebutton.isEnabled();
	       if (email_field_errorMessage.isDisplayed()) {
	    	   
	     String mesg = email_field_errorMessage.getText();
		 ...
		 System.out.println("Email field Error message is showing when user enter interger and spe symbol value " );
			...
			Reporter.log("Passed- Verify user should not enter intger value  in mobile number field.",true);
