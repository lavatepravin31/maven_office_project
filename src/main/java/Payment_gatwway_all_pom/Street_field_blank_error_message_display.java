...
public class Street_field_blank_error_message_display {
...
WebDriver driver;
	...
	@FindBy(xpath = "//mat-error[text()='Enter valid Street Address']")
	WebElement Streetfield_errorMessage;
	
	
	public Street_field_blank_error_message_display(WebDriver driver) {
		this.driver=driver;
	}
	...
	public void enterDATA() throws InterruptedException {
	    ...
	    Reporter.log("Street field is blank",true);
	...
	public void streetfield_error_message() {
       if (Streetfield_errorMessage.isDisplayed()) {
			
    	   String mesg = Streetfield_errorMessage.getText();
    	   System.out.println("Error message is  :: "+mesg);
		   System.out.println("street Error message is showing when its blank " );
				
		}
		else {
			System.out.println("street  Error message is not showing ");
		}
       
       String expected_result="Enter valid Street Address";
       String mesg = Streetfield_errorMessage.getText();
       String actual_result=mesg;
       Assert.assertEquals(expected_result, actual_result, "Error message is not match");
       Reporter.log("Pass- Check in Street field filed, mandatory field is marked with a red asterisk symbol.",true);		
