...
public class Country_dropdown_blank_error_message {
...
WebDriver driver;
	...
	@FindBy(xpath = "//mat-error[text()='Select Country']")
	WebElement coutrydropdown_errorMessage;
	
	
	public Country_dropdown_blank_error_message(WebDriver driver) {
		this.driver=driver;
	}
...
//		Thread.sleep(2000);
	...
	public void countrydropdown_error_message() {
       if (coutrydropdown_errorMessage.isDisplayed()) {
    	   String mesg = coutrydropdown_errorMessage.getText();
    	   System.out.println("Error message is  :: "+mesg);
			
			System.out.println("country Error message is showing when its blank " );
				
		}
		else {
			System.out.println("country Error message is not showing ");
		}
       

       String expected_result="Select Country";
       String mesg = coutrydropdown_errorMessage.getText();
       ...
       Reporter.log("Pass- Verify that  without selecting dropdown,  its should  show error message.",true);		
