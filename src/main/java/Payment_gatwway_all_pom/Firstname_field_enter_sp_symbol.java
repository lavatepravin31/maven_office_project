...
public class Firstname_field_enter_sp_symbol {
WebDriver driver;
	...
	WebElement firstname_field_errorMessage;
	
	
	
	public Firstname_field_enter_sp_symbol(WebDriver driver) {
		this.driver=driver;
	}
	...
	public void enterDATA() throws InterruptedException {
		...
		CVV.sendKeys("465");
		FIRSTname.sendKeys("@#$%^&*&");
		Reporter.log("Enter Sp symbol in First name field ",true);
	...
	public void Firstname_error_message() {
	       if (firstname_field_errorMessage.isDisplayed()) {
	    	   String mesg = firstname_field_errorMessage.getText();
	    	   System.out.println("Error message is  :: "+mesg);
				
				System.out.println("firstname field Error message is showing when user enter sp symbol " );
				Reporter.log("PAss-firstname field Error message is showing when user enter sp symbol ",true);	
			}
			else {
				System.out.println("firstname field  Error message is not showing ");
				Reporter.log("failed-firstname field Error message is showing when user enter sp symbol ",true);
			}
}
	}
